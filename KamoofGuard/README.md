...
### `/playerban <joueur> [raison...] [durée]` (`TempBan`)

`/playerban` bannit comme `/ban`, mais accepte une **durée en dernier argument** : `10s`, `3m`,
`2h`, `1d`, `1w`, `1y`, ou une suite (`1d12h`). Insensible à la casse. **Sans ce dernier argument,
le ban est définitif.** Le `/ban` vanilla n'est plus touché du tout et reste utilisable tel quel
(définitif uniquement).

```
/playerban Toto                          -> définitif
/playerban Toto triche au combat         -> définitif, raison "triche au combat"
/playerban Toto 10m                      -> 10 minutes
/playerban Toto grief repete 1D12H       -> 36 heures, raison "grief repete"
```

La commande n'est pas réimplémentée. Elle retire l'argument de durée puis délègue à
`minecraft:ban` avec le sender d'origine — c'est donc le vanilla qui vérifie la permission, résout
la cible (sélecteurs `@a`/`@p` compris), expulse le joueur et écrit `banned-players.json` — puis
...
- un mot de raison qui ressemble à une durée (`/playerban Toto grief 3d`) est bien lu comme une
  durée ;
- `punish-command` peut donc valoir `playerban %player% triche (%check%) 7d`.

### Permissions
`kguard.alerts` (op) · `kguard.command` (op) · `kguard.ban` (op) · `kguard.bypass` (personne)
